...
- 06d6c1f: New `SplitButton` component — a primary action with an attached chevron menu of secondary choices. Declarative `SplitButtonItem[]` model (label, optional description second line, lucide icon, `disabled`/`hidden`, `separatorBefore`), required `menuLabel` for an accessible trigger name, `variant`/`size`/`fullWidth`/`align` options. When every item is hidden it renders a plain button. Composes the existing Button and DropdownMenu primitives; exposes `splitButtonVariants` and segment/item variant cvas.

## 0.22.1

### Patch Changes

- c5a05a3: Left-anchor the `narrow` PageContent width variant. Narrow pages (forms,
...
- 9b29b02: New `Carousel` and `ImagePreview` components for in-app media preview.
