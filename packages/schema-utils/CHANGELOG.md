...
- 233fc86: Remove `createViteAppConfig`, `appConfigSchema`, and `siteConfigSchema` — env/runtime config now lives in the dedicated `@bota-apps/app-config` package. Import `createViteAppConfig` (and the new `graphqlAppEnv` base schema) from `@bota-apps/app-config` instead. schema-utils stays focused on form/detail/currency schema contracts.
...
- 11a8bb6: Add `translatedOptions(options, t, prefix)` (on the `/localize` + `/i18n` subpaths) — the generated-list counterpart of `translatedOptionsFromEnum`, for translating select/filter options when you already hold the SDL-derived `<enum>Options` list rather than a Zod enum. Labels resolve from `${prefix}.${value}`.

### Patch Changes

- 6cfe6e1: Add `sumMoney(values, fallbackCurrency)` — reduces a list of `Money` values to a single total (single-currency; uses the first value's currency, or the fallback for an empty list). Patch (additive, stays in the `^0.x` range dependents pin).

## 0.5.0

### Minor Changes

- a7c25fd: New `createViteAppConfig(schema, env)`: resolves a Zod-validated config object from Vite env vars — each schema key reads `VITE_<SCREAMING_SNAKE>` (`graphqlUrl` → `VITE_GRAPHQL_URL`), Vite builtins map through unprefixed (`mode` → `MODE`). Throws at startup on missing or invalid values.
