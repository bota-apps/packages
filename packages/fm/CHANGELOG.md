...
- 11a8bb6: Add the generic capability/RBAC adapters host apps kept re-implementing:

  - `capabilitiesToFeatureInputs(capabilities, planTierOrder)` — maps a platform org-capabilities object (flags/limits/setupCompletion/plan) into the flat gating inputs `useFeatureSetup` consumes (flag entries, plan-tier expansion, reached/approaching limits, completed setup). Only `planTierOrder` is app data.
  - `buildResourceTree` / `pruneToGranted` / `collectResourceIds` (+ `FlatResourceNode` / `ResourceTreeNode<T>` types) — rehydrate the flat, `parentId`-linked RBAC resource catalog into a nested tree, prune it to granted ids, and walk it. Generic over the node payload, so an app's own resource type passes through intact.
