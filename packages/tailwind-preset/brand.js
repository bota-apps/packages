 ...
 * import order never matters. A brand redefines only the chromatic tokens
 * (primary/accent ramps, ring, sidebar highlights, the matching chart slots);
 * the neutrals inherit from theme.css so every brand keeps the same surfaces.
 */
...
${rampLines("accent", colorRamp(accent))}
  ...
  --chart-3: ${channels(accentHsl)};${shapeBlock}
}
...
:root[data-brand="${name}"].dark {
  ...
  --chart-3: ${channels(lightened(accentHsl, 7))};
}
`;
}
